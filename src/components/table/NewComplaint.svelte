<script lang="ts">
	...
	import getNewComplaints from '../../functions/NewComplaints';
	...
	const defaultColumns: ColumnDef<Complaints>[] = [
		{
			...
			cell: (info: any) =>
				renderComponent(LinkComponent, {
					link: info.cell.row.getValue('docId'),
					data: info.getValue(),
					isDownload: false
				})
		},
		...
		},
		{
			...
			cell: (info) =>
				renderComponent(LinkComponent, {
					link: info.getValue() !== '' ? info.getValue() : info.cell.row.getValue('docId'),
	...
	onMount(() => {
		user.subscribe(async (val) => {
			const data = await getNewComplaints(val?.uid);
